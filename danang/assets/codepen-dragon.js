// Adapted from the user-provided CodePen Processing.js dragon sketch.
...
window.daNangDragonSketch = function(processingInstance) {
     with (processingInstance) {
        ...
        var clearDaNangCanvas = function() {
            if (processingInstance.externals && processingInstance.externals.context) {
                var ctx = processingInstance.externals.context;
                ctx.save();
                if (ctx.setTransform) {
        ...
        var prepareDaNangDragon = function() {
            scene.page = "go";
            scene.loaded = true;
            scene.showClouds = false;
            scene.showClick = false;
            scene.dragon = new Dragon();
            scene.dragon.x = 200;
            scene.dragon.y = 205;
            scene.dragon.speed = 4;
            scene.dragon.lift = 3;
            scene.dragon.colors = scene.dragon.colorThemes.red;
        };

        window.__resetDaNangDragon = prepareDaNangDragon;
        prepareDaNangDragon();
